...
import demo.couchbase.part2.domain.Identificable;
...
public abstract class AbstractCrudServiceImpl<T extends Identificable> implements CrudService<T> {

    private static final Logger logger = Logger.getLogger(AbstractCrudServiceImpl.class.getName());

    private BucketService bucketService;
    private Bucket bucket;
    private JsonConverter<T> jsonConverter;

    public AbstractCrudServiceImpl(BucketService bucketService, JsonConverter<T> jsonConverter) {
    ...
    public void create(T t) {
        ...
        JsonDocument doc = jsonConverter.toDocument(t);
        bucket.insert(doc);
    }
    ...
    public T read(String id) {
        ...
        return (doc == null ? null : jsonConverter.fromDocument(doc));
    }

    @Override
    public T readFromReplica(String id) {
        List<JsonDocument> docs = bucket.getFromReplica(id, ReplicaMode.FIRST);
        return (docs.isEmpty() ? null : jsonConverter.fromDocument(docs.get(0)));
    }
        ...
        try {
            asyncOperation.toBlocking().forEach(new Action1<JsonDocument>() {
                public void call(JsonDocument doc) {
                    T item = jsonConverter.fromDocument(doc);
                    items.add(item);
                }
            });
        } catch (Exception e) {
            logger.info("Error during bulk get " + e);
        }
